# ashare-monitor 项目总结（v4：四市场投研 + 低频交易 + 企业级连接器回填）

> 个人投研工作站的完整进化：A 股 / 港股 / 美股 / 数字货币四市场一体化
> 监控、选股、回测与低频自动化交易模拟，并完成三轮同类开源项目借鉴、
> 工程化升级（配置驱动 / 状态机 / 可编程 / 自文档 / Agent 可查询）与
> 企业级数据连接器回填（Wind / 天眼查 / 智慧芽）。
> **117 commits · 353 测试 · 54 CLI 命令 · 22 API 函数 · 23 MCP 工具 · 58 模块**（2026-08-25）
...
| 企业画像/知识产权 | ✅ 5 家已回填（天眼查/智慧芽）| ⚠️ 小米已回填 | ⚠️ 英伟达已回填 | — |
...
| 数据源 | 回填内容 | 落地 |
|---|---|---|
| Wind | K 线校准（前复权日 K）| `import_klines_json` → klines 表（幂等）；比亚迪/茅台/平安/宁德均补至最新交易日 |
| 天眼查 | 工商画像（规模/标签/行业/注册资本/曾用名）| `import_company_profile` → company_profiles 表（upsert）；5 家：比亚迪/茅台/平安/宁德/小米 |
| 智慧芽 | 专利 + 论文（含 legal_status/IPC/产学研）| `import_ip_assets` → ip_assets 表（按 pn/title 合并去重）；6 家：比亚迪/茅台/平安/宁德/小米/英伟达 |

**A 股体检 22 维**（新增 15a 工商画像 / 15a2 知识产权 / 15b 两融 / 15c 解禁）：
行情、K 线历史、择时、基本面、估值分位、行业、研报数、事件、基金持仓、筹码、
增减持、龙虎榜、股权质押、**工商画像**、**知识产权（专利/论文）**、**两融**、**解禁**等。

**横向对比报告**：`docs/patent_landscape.md`（6 家专利池/技术聚焦/产学研自动化生成，
如实标注采样边界）。
...
| **天眼查（工商画像）** | ✅ 会话内 MCP | 5 家画像入库，check 模糊匹配展示 |
| **智慧芽（专利/论文）** | ✅ 会话内 MCP | 6 家入库，采样快照（如实标注）|
...
- 三源回填：Wind 7 日与 akshare 逐点一致；天眼查 5 家画像；智慧芽 6 家专利/论文（对比报告见 docs/patent_landscape.md）
...
- 待办：实盘通道（QMT/PTrade，券商开通 + 合规报备）· 模拟运行 3 个月验证 ·
  链上数据本机直连验证 · 专利质量升级（有效/申请中/PCT/IPC 分布入体检）·
  画像新鲜度追踪（>90 天提示回补）· CI 覆盖率徽章
